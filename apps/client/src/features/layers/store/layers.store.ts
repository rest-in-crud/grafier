...
import { LayersStoreInterface } from '@/features/layers/store/layers.store.interface.ts';
...
export const useLayersStore = create<LayersStoreInterface>((set) => ({
  ...
  moveObjectBetweenLayers: (objectId, fromLayerId, toLayerId) =>
    set((state) => ({
      layers: state.layers.map((l) => {
        if (l.id === fromLayerId) return { ...l, objectsIds: l.objectsIds.filter((id) => id !== objectId) };
        if (l.id === toLayerId) return { ...l, objectsIds: [...l.objectsIds, objectId] };
        return l;
      }),
    })),
